...
@RestController
@RequestMapping("/api/estado-equipos")
public class EstadoEquipoController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<EstadoEquipo> getById(@PathVariable Integer id) {
    ...
    @PostMapping
    public ResponseEntity<EstadoEquipo> create(@RequestBody EstadoEquipo entity) {
        EstadoEquipo saved = service.save(entity);
        return ResponseEntity.status(HttpStatus.CREATED).body(saved);
    }

    @PutMapping("/{id}")
    public ResponseEntity<EstadoEquipo> update(@PathVariable Integer id, @RequestBody EstadoEquipo entity) {
        if (service.findById(id).isEmpty()) {
            return ResponseEntity.notFound().build();
        }
        entity.setIdEstadoEquipo(id);
